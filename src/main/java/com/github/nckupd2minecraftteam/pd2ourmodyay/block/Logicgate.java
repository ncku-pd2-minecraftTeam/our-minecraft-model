...
public class Logicgate extends RedstoneDiodeBlock {
    ...
    public Logicgate(BiFunction<Boolean, Boolean, Boolean> calculateOutputFunction) {
    ...
    public boolean canConnectRedstone(BlockState state, IBlockReader world, BlockPos pos, @Nullable Direction side) {
        return side == state.get(Logicgate.HORIZONTAL_FACING) || side == state.get(Logicgate.HORIZONTAL_FACING).rotateY() || side == state.get(Logicgate.HORIZONTAL_FACING).rotateYCCW();
    }

    @Override
    protected int calculateInputStrength(World world, BlockPos pos, BlockState state) {
        Direction facing = state.get(Logicgate.HORIZONTAL_FACING);
